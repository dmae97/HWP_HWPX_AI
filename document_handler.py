...
class DocumentProcessorFactory:
    ...
    def create_handler(file_path: str = None, file_obj: BinaryIO = None, 
        ...
        """
        파일 형식과 플랫폼에 따라 적절한 문서 처리기를 생성합니다.
        
        Args:
            file_path: 파일 경로 (선택 사항)
            file_obj: 이진 파일 객체 (선택 사항)
            file_type: 파일 형식 (선택 사항, 자동 감지 가능)
            api_keys: API 키 딕셔너리 (선택 사항)
        ...
        if file_type is None:
            if file_path:
                ...
                file_type = os.path.splitext(file_obj.name)[1].lower().replace(".", "")
            else:
                # 파일 매직 바이트로 추정 (필요시 구현)
                file_type = "unknown"
